"""
stabilize_video.py -- STABLE video of the fly hovering and getting poked.
================================================================================
The validated setup: airborne release (z = 0.4, upright), gentle flapping at
amp 0.25 (sub-stall, near-hover thrust), 3-DOF wings at 200 Hz, brain attached
(gain 8). At t = 1.5 s we poke the brain's right-side sensory pools for 0.4 s
-> descending neurons fire -> legs kick -> the fly keeps hovering.

CAMERA: non-rotating chase cam (free camera, fixed azimuth/elevation/distance,
lookat = fly position). The fly stays centered, the camera never orbits.

Run:    MUJOCO_GL=glfw xvfb-run -a python stabilize_video.py
Output: stabilize_hover_poke.gif  (+ prints metrics)
"""
import os
import sys
import numpy as np
...
m.opt.timestep = 5e-5                       # 10 samples per wingbeat @ 200 Hz
...
CLAMP = 1.0
TOUCH_GAIN = 0.05
FRAME_EVERY = int(0.03 / DT)                # 30 ms sim per video frame

# ============================ STATE ============================
d = mujoco.MjData(m)
d.qpos[0] = 0.0; d.qpos[1] = 0.0; d.qpos[2] = 0.4     # airborne release
...
# ============================ RENDERER (non-rotating chase cam) ============================
renderer = None
frames = []
if VIDEO:
    try:
        ...
        cam.azimuth = 90.0
        cam.elevation = 25.0
        print("renderer ready (chase cam, fixed orientation)")
...
for step in range(int(T / DT)):
    ...
    # gentle flapping, soft ramp (0.5 s)
    ramp = min(1.0, t / 0.5)
    amp_t = BASE_AMP * ramp
    w = 2 * np.pi * FLAP_F * t
    for side in ("left", "right"):
        d.ctrl[wing_a[side]["pitch"]] = amp_t * np.sin(w)
        d.ctrl[wing_a[side]["roll"]] = 0.6 * amp_t * np.sin(w + np.pi / 2)
        d.ctrl[wing_a[side]["yaw"]] = 0.3 * amp_t * np.sin(w + np.pi)
    ...
    # capture frame (chase cam: lookat = fly, fixed orientation -> no spin)
...
if renderer is not None and frames:
    ...
    gif[0].save("stabilize_hover_poke.gif", save_all=True, append_images=gif[1:],
                duration=int(FRAME_EVERY * DT * 1000), loop=0)
    print(f"saved stabilize_hover_poke.gif ({len(frames)} frames, "
          f"{FRAME_EVERY * DT * 1000:.0f} ms/frame)")
else:
    print("no video (renderer unavailable) -- simulation still ran")

zs = np.array(zs); tilts = np.array(tilts)
print(f"desc spikes during poke: {desc_during}")
print(f"max tilt: {tilts.max():.1f} deg | tilt@end: {tilts[-3:].mean():.1f} deg | "
      f"z@end: {zs[-3:].mean():+.3f} m | NaN: {not np.isfinite(zs).all()}")
